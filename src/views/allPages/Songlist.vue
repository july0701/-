<template>
  <div>
    <div class="Cover">
      <div :style="{'background-image' : 'url(' + tuijianlist.coverImgUrl + ')'}" class="cover_bg"></div>
      <div class="main">
        <div style="position: relative;" class="main_left">
          <img class="coverImgUrl" :src="tuijianlist.coverImgUrl" alt />
          <div class="Listens">
            <van-icon name="audio" />
            <span>{{ tuijianlist.playCount }}</span>
          </div>
          <b class="top_icons">歌单</b>
        </div>
        <div class="gedanname">
          <p style="font-size: 17px;">{{ tuijianlist.name }}</p>
          <p
            @click="goUsercardTop(tuijianlist.userId)"
            style="position:relative;margin-top:10%;width:200px;height:30px;line-height:30px;"
          >
            <img
              style="width:30px;height:30px;border:1px solid #fff;border-radius:50%;margin-right:6px;"
              :src="name_avatarUrl"
              alt
            />
            <img ref="icon" class="user_icon" :src="user_icon" alt />

            <span style="color:#fff;font-size:13px;">{{name_nickname}}</span>
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="list_top">
      ...
      <!-- 查看更多歌曲，请下载客户端 -->
      <div @click="openDown" class="lokkMore">
        <p>查看更多歌曲，请下载客户端</p>
      </div>
    </div>
    <!-- 评论 -->
    ...
    <div class="Collection">
      <div @click="openDown" class="Collection_con">
        <van-icon size="24px" name="music-o" />收藏歌单
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getTuijian() {
      ...
      this.$http.get("http://39.106.201.40:3000/playlist/detail", { params: { id } }).then((res) => {
        console.log(res, "当前推荐歌单数据");
        if (res.status == 200 && res.statusText == "OK") {
          this.tuijianlist = res.data.playlist;
          this.name_avatarUrl = res.data.playlist.creator.avatarUrl;
          this.name_nickname = res.data.playlist.creator.nickname;
          if (res.data.playlist.creator.avatarDetail !== null) {
            this.$refs.icon.style.display = "block";
            this.user_icon =
              res.data.playlist.creator.avatarDetail.identityIconUrl;
          } else {
            this.$refs.icon.style.display = "none";
          }

          res.data.playlist.trackIds.slice(0, 20).forEach((n) => {
            this.songsListID.push(n.id);
          });
          console.log(this.songsListID, "歌单id");
          this.getSongs(this.songsListID.join(",")); //调用歌单歌曲
          this.getComments(res.data.playlist.id); //调用评论
          if (this.tuijianlist.playCount > 10000) {
            //换单位
            let wan = this.tuijianlist.playCount / 10000;
            this.tuijianlist.playCount = wan.toFixed(1) + "万 ";
            if (wan > 10000) {
              let yi = wan / 10000;
              this.tuijianlist.playCount = yi.toFixed(1) + "亿";
            }
          }
        }
      });
    ...
    getComments(listId) {
      this.$http
        .post("http://39.106.201.40:3000/comment/playlist?id=" + listId)
        .then((res) => {
    ...
    getss() {
      this.$http
        .get("http://39.106.201.40:3000/comment/playlist?id=705123491")
        .then((res) => {
          console.log(res, "点击推荐歌单 调数据用");
        });
    },
    //播放
